...
import postPhone, { ERROR } from '@/api/signUp/postPhone';

import doSetPhone from '../doSetPhone';
import entity from './phone';

const { error: accessError } = content.phone;

/**
 * Action which submits a form of sign up access by phone.
 */
export default (history: any, confirmLink: string) => async (dispatch: any, getState: any) => {
  ...
  try {
    ...
    dispatch(doSetPhone(value, history));
  } catch (e) {
    if (e.message !== ERROR) {
      throw e;
    }
